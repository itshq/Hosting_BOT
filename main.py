...
TOKEN = '8523721125:AAEogQ85JUlXR6UTxY_dfoTP7RjzwPaJvP0'
if not TOKEN:
...
def locked_msg(chat_id):
    ...
    markup.add(types.InlineKeyboardButton("👨‍💻 المطور", url="https://t.me/its_h_q"))
...
def start_cmd(msg):
    try:
        ...
        users = read_json(USERS_DB)
        if uid != ADMIN_ID and users.get(str(uid), {}).get('is_banned', 0) == 1:
...
def callback(call):
    try:
        ...
        if is_bot_locked() and not is_admin(uid):
            ...
            locked_msg(cid)
            return
        if uid != ADMIN_ID and str(uid) in users and users[str(uid)].get('is_banned', 0) == 1:
        ...
        if data == "nav_main":
            ...
            if int(tuid) == ADMIN_ID:
                bot.answer_callback_query(call.id, "❌ خط أحمر! لا يمكن تعديل صلاحيات المطور الأساسي.", show_alert=True)
                return
...
def pro_grant_step(msg, tuid, prompt_id):
    ...
    del_msg(msg.chat.id, prompt_id, msg.message_id)
    if int(tuid) == ADMIN_ID:
        send_msg(msg.chat.id, deco("❌ خطأ", "المطور خط أحمر!"), back_kb(f"uctrl_{tuid}"))
        return
    if not msg.text or not msg.text.strip().isdigit():
        send_msg(msg.chat.id, deco("❌ خطأ", "رقم غير صحيح!"), back_kb(f"uctrl_{tuid}"))
        return
...
def ban_toggle(call, tuid):
    if int(tuid) == ADMIN_ID:
        bot.answer_callback_query(call.id, "❌ خط أحمر! لا يمكنك حظر المطور الأساسي.", show_alert=True)
        return
    users = read_json(USERS_DB)
    if str(tuid) in users:
...
def pro_remove(call, tuid):
    if int(tuid) == ADMIN_ID:
        bot.answer_callback_query(call.id, "❌ خط أحمر! لا يمكن مسح صلاحيات المطور الأساسي.", show_alert=True)
        return
    users = read_json(USERS_DB)
    if str(tuid) in users:
...
def settings_panel(call):
    ...
    text = f"✏️ اسم البوت: {settings.get('bot_name', 'Div: @its_h_q')}\n🖼 صورة البوت: {has_img}\n🎨 أيقونة الملفات: {has_thumb}\n✅ موافقة تلقائية: {auto_approve}"
    kb = types.InlineKeyboardMarkup(row_width=1)
